...
def ssh_pillar_tests_prep(request, salt_master, salt_minion):
    ...
    log.warning("RET: %s", pprint.pformat(ret))
...
def webserver_pillar_tests_prep(request, salt_master, salt_minion):
    ...
    request.cls.nginx_port = nginx_port = get_unused_localhost_port(cached_seconds=120)
    uwsgi_port = get_unused_localhost_port(cached_seconds=120)
    ...
    try:
        ret = salt_call_cli.run("state.apply", mods="git_pillar.http", pillar=pillar)
        assert ret.exitcode == 0
        assert next(iter(ret.json.values()))["result"] is True
